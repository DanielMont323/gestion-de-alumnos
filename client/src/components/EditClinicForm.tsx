...
const EditClinicForm: React.FC<EditClinicFormProps> = ({ clinic, onClose, onSuccess }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await clinicsAPI.update(clinic._id, {
        name,
        address,
        groups
      });
      ...
      setError(err.response?.data?.message || 'Error al actualizar la clínica');
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between p-6 border-b sticky top-0 bg-white z-10">
          <h2 className="text-xl font-bold text-gray-800">Editar Clínica</h2>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div className="pt-4 border-t sticky bottom-0 bg-white">
            <div className="flex gap-3 justify-end">
              ...
              >
                {isLoading ? 'Actualizando...' : 'Actualizar Clínica'}
              </button>
            </div>
          </div>
        </form>
